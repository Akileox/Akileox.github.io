...
- 이 오차 누적은 BC(behavior cloning)의 covariate shift와 같은 구조다. MDN-RNN은 학습 때 항상 진짜 관측된 \\( z_t \\)를 입력으로 쓰지만(teacher forcing), 상상 롤아웃에서는 자기 자신이 예측한 \\( \hat z_t \\)를 다음 입력으로 재사용해야 한다. 학습 때 한 번도 보지 못한, 자기 오차가 섞인 입력을 처음 만나는 것이다. 시퀀스 모델링에서는 이 현상을 exposure bias라 부르고, DAgger의 발상을 가져온 scheduled sampling(학습 중에도 가끔 모델 자신의 예측을 입력으로 섞는 것)이 대표적인 해법이다. 이 편의 구조에는 그런 보정이 전혀 없다.

위에서 말한 model bias는 고전적인(tabular) Model-Based RL이 고차원에서 겪는 문제와는 구분해야 한다. Value iteration/Policy iteration 같은 방법은 \\( T(s'\mid s,a) \\), \\( R(s,a) \\)를 표(table)로 명시적으로 갖고 있어야 한다. 이 표의 크기 자체가 상태 변수 개수에 대해 기하급수적으로 커지고(차원의 저주), 표를 만드는 것도 그 표 전체를 훑는 DP 업데이트도 계산이 불가능해진다. Model-Free(Q-learning 등)가 역사적으로 이걸 우회한 이유가 이거다. T, R을 아예 표로 만들지 않고, 샘플링된 경험만으로 \\( Q(s,a) \\)를 직접 학습하기 때문이다.

World Models/Dreamer가 겪는 문제는 이것과 다르다. 여기서 T, R은 표가 아니라 신경망이다. 신경망은 애초에 고차원 연속 공간을 표로 만들지 않고 일반화하려고 쓰는 도구라서, "표가 너무 커서 계산이 안 된다"는 문제 자체가 없다. 대신 생기는 문제는 학습된 신경망 근사가 얼마나 정확한가, 그리고 그 근사 오차가 상상 롤아웃을 반복할수록 얼마나 누적되는가다. 계산 가능/불가능의 문제가 아니라 통계적 근사 오차와 그 오차의 전파(compounding) 문제인 셈이다. 둘 다 "Model-Based는 고차원에서 대가를 치른다"는 큰 흐름 안에 있지만, 하나는 명시적 표현이 계산상 불가능해지는 문제(고전 tabular RL의 차원의 저주)고 다른 하나는 학습된 근사가 부정확해지고 그 오차가 누적되는 문제(딥러닝 시대 model-based RL의 model bias)라 서로 다른 문제다.
