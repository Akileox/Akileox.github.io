...
이 논문의 핵심 아이디어는 객체 이름 뒤에 `<sam_pad>`라는 특수 토큰을 붙여서, VLM이 텍스트를 생성하는 과정 중에 이 토큰을 자연스럽게 출력하도록 학습시키는 것이다. 이 토큰의 잠재 임베딩 $$z$$는 선형 레이어를 거쳐 SAM 프롬프트 공간의 벡터 $$z_m = Wz + b$$로 변환되고, 다시 객체 클래스별 학습 가능한 쿼리 $$q$$를 이용한 크로스 어텐션

$$z_p = \text{Attn}(q, k, v) = \text{softmax}\left(\frac{q k^\top}{\sqrt{d_k}}\right) v$$

을 거쳐 SAM 디코더의 입력 프롬프트로 전달된다. 여기서 $$k, v$$는 모두 $$z_m$$에서 나온다. 이 구조가 재미있는 건 `<sam_pad>` 토큰이 세그멘테이션을 위한 입력으로만 쓰이는 게 아니라, 뒤이어 생성되는 답변 토큰들이 인과적 어텐션(causal attention)을 통해 이 토큰을 다시 참조할 수 있다는 점이다. 즉 같은 토큰이 한 번은 SAM 쪽으로, 한 번은 답변 생성 쪽으로 두 갈래로 쓰인다.
...
가장 설득력 있는 검증은 벤치마크 수치보다 어텐션 시각화 쪽이었다. EndoVis18과 RAMIE 데이터셋에서 답변을 생성할 때 VLM 내부 어텐션 맵을 보면, 답변 토큰이 앞서 생성된 여러 객체 토큰 중에서 의미적으로 관련 있는 토큰(예: 실제로 질문에 등장하는 수술 도구나 조직에 대응하는 `<sam_pad>` 토큰)에 뚜렷하게 높은 가중치를 준다.

![답변 토큰이 이전에 생성된 객체 토큰들 중 의미적으로 관련된 토큰에 높은 어텐션 가중치를 부여하는 것을 보여주는 시각화](/assets/images/posts/object-tokens-as-a-bridge-between-segmentation-and-visual-question-answering-in-/figure-2.png)
