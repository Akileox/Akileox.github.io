---
title: "World Model은 학습에만, 추론엔 버리자"
date: 2026-08-10
categories: [AI]
...
이 부분은 사실 요약이 아니라 제 의견입니다. 이 논문은 World Model을 학습 때만 쓰고 추론 때는 버린다는 실용적인 트레이드오프를 아주 깔끔하게 구현한 사례라서, World Model을 매개로 표현을 이식하려는 모든 프로젝트가 결국 부딪힐 질문에 하나의 실증적 답을 제시한다는 인상을 받았다.

<details class="tc-faq">
<summary>Isolated Attention Mask가 정말 필요한 만큼의 지식만 새어 들어가게 하는지, 아니면 학습 초반/후반에 정보 흐름이 달라지는 동역학이 숨어 있는지 궁금하다.</summary>
<div class="tc-faq__body" markdown="1">

SimWAM은 별도의 distillation 단계 없이 공동 학습과 attention mask 하나로 지식 이식 문제를 해결했지만, 논문에서는 이 마스크의 정보량이 학습 단계별로 어떻게 변하는지를 깊게 다루지 않는다.

</div>
</details>

<details class="tc-faq">
<summary>이 구조가 만약 cross-embodiment 세팅으로 확장된다면 taskcraft에 그대로 적용할 수 있을까.</summary>
<div class="tc-faq__body" markdown="1">

Video DiT가 사람의 시연 영상을 담당하고 Action DiT가 로봇 관절을 담당하며 둘 사이를 attention으로만 연결한다면 taskcraft가 상정하는 공유 latent와 embodiment별 디코더 구조와 거의 같은 모양이 된다. 다만 자율주행은 카메라 시점과 행동 공간이 이미 상당히 정렬된 비교적 닫힌 문제라서, 조류형 로봇과 사람 손처럼 몸의 형태 자체가 근본적으로 다른 경우에도 이 attention 기반 분리가 버텨줄지는 훨씬 회의적으로 봐야 한다. 이 논문이 푼 건 같은 몸 안에서 감각과 행동을 분리하는 문제이지, 다른 몸으로 지식을 옮기는 문제는 아니라는 구분을 분명히 해두고 싶다.
