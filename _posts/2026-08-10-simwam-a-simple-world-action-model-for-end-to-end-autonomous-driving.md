...
이 부분은 사실 요약이 아니라 제 의견입니다. 이 논문은 세계 모델을 학습 때만 쓰고 추론 때는 버린다는 실용적인 트레이드오프를 아주 깔끔하게 구현한 사례라서, 세계 모델을 매개로 표현을 이식하려는 모든 프로젝트가 결국 부딪힐 질문에 하나의 실증적 답을 제시한다는 인상을 받았다.
